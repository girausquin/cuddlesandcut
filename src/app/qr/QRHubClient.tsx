...
const WHATSAPP_NUMBER =
  process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? "+1 (512) 983-1650";
const IG_URL =
  process.env.NEXT_PUBLIC_IG_URL ?? "https://instagram.com/cuddlesandcut";
const FB_URL =
  process.env.NEXT_PUBLIC_FB_URL ?? "https://facebook.com/Cuddlesandcut";
...
// ====== UI ======
export default function QRHubClient() {
  ...
  return (
    <main className="relative min-h-[88vh] w-full overflow-hidden">
      ...
      <section className="relative mx-auto max-w-5xl px-6 pt-14 pb-16">
        <motion.div initial="hidden" animate="show" transition={{ staggerChildren: 0.05 }} className="text-center">
          <motion.h1 variants={fade} className="text-4xl md:text-5xl font-extrabold tracking-tight">
            Cuddles & Cuts
          </motion.h1>
          <motion.p variants={fade} className="mt-3 text-base md:text-lg text-gray-600">
            Love in one hand, style in the other. ✂️💚
          </motion.p>
          <motion.p variants={fade} className="mt-2 text-sm text-gray-500">
            Scan → pick your favorite channel → book in seconds.
          </motion.p>
        </motion.div>

        <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
          <ContactCard label="WhatsApp" href={waHref} Icon={WAIcon} accent="from-emerald-400 to-teal-500" />
          <ContactCard label="Instagram" href={igHref} Icon={IGIcon} accent="from-pink-400 to-fuchsia-500" />
          <ContactCard label="Facebook" href={fbHref} Icon={FBIcon} accent="from-blue-500 to-indigo-600" />
        </div>
      </section>
    </main>
  );
    ...
    >
      ...
      <div className="relative rounded-3xl bg-white/70 backdrop-blur-xl p-6 h-full shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <div className="mt-1 text-xs text-gray-500">Tap to open {label}</div>
        </div>
      </div>
      <div
