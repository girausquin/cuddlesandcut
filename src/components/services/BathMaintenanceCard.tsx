...
import { getTXT, getFEATURES, TIERS, DISCOUNT, priceNoCut } from "./constants";
...
export default function BathMaintenanceCard() {
  const { locale } = useLocale(); // 👈 idioma actual
  const TXT = getTXT(locale); // 👈 textos traducidos
  ...
  // Quitamos el icono de tijeras (solo baño)
  const featuresNoCut = useMemo(
    () => FEATURES.filter((f) => f.icon !== Scissors),
    [FEATURES]
  );
  ...
  return (
    <motion.article
      aria-labelledby="svc-nocut-title"
      aria-describedby="svc-nocut-desc"
      variants={fadeInUp}
      initial="hidden"
      whileInView="show"
      viewport={{ once: true, amount: 0.25 }}
      className="relative z-10 mx-auto mt-10 w-full rounded-3xl border border-black/10 bg-white p-6 sm:p-8 shadow-xl"
    >
      {/* Título */}
      ...
      <p id="svc-nocut-desc" className="sr-only">
        {TXT.includes} {TXT.priceByWeight}
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start">
        ...
        <div ref={includesRef}>
          ...
          <FeatureList features={featuresNoCut} />
        </div>
          ...
          >
            {tiers5.map((t) => (
              ...
              >
                <PriceCard
                  label={t.label}
                  price={priceNoCut(t.base, DISCOUNT)}
                />
              </motion.div>
            ))}
          </motion.div>
